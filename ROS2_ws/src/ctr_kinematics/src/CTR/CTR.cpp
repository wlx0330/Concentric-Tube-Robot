...
void CTR::SolveShape()
{
	...
	for (int i = 0; i < this->bc.m; ++i)
	{
		...
		y = this->bvp.y_mesh[i];
		blaze::row(u, 0UL) = (K_z[0] * u_abs[0] * blaze::cos(blaze::row(y, 0UL)) + K_z[1] * u_abs[1] * blaze::cos(blaze::row(y, 1UL)) + K_z[2] * u_abs[2] * blaze::cos(blaze::row(y, 2UL))) / blaze::sum(K_z);
		blaze::row(u, 1UL) = (K_z[0] * u_abs[0] * blaze::sin(blaze::row(y, 0UL)) + K_z[1] * u_abs[1] * blaze::sin(blaze::row(y, 1UL)) + K_z[2] * u_abs[2] * blaze::sin(blaze::row(y, 2UL))) / blaze::sum(K_z);
	...
	const blaze::DynamicMatrix<double> &y_int)
{
	...
	blaze::DynamicVector<double, blaze::rowVector> height = blaze::subvector(x_int, 1UL, x_int.size() - 1) - blaze::subvector(x_int, 0UL, x_int.size() - 1);
	blaze::DynamicMatrix<double, blaze::rowMajor> base = blaze::submatrix(y_int, 0UL, 0UL, 3UL, y_int.columns() - 1) + blaze::submatrix(y_int, 0UL, 1UL, 3UL, y_int.columns() - 1);
	// calculate area
	for (int i = 0; i < 3; ++i)
	{
	...
	base = blaze::submatrix(p_dot, 0UL, 0UL, 3UL, y_int.columns() - 1) + blaze::submatrix(p_dot, 0UL, 1UL, 3UL, y_int.columns() - 1);
	for (int i = 0; i < 3; ++i)
	{
		//check speed
		blaze::row(area, i) = blaze::row(base, i) * height * 0.5;
...
void CTR::SolveBC(const blaze::StaticVector<double, 3UL> &TubeTran)
{
	...
	BV = {{-CTR::tubes[0].GetLS(), -CTR::tubes[1].GetLS(), -CTR::tubes[2].GetLS()},
		  {0, 0, 0},
		  {CTR::tubes[0].GetLC(), CTR::tubes[1].GetLC(), CTR::tubes[2].GetLC()}};
...
// get CTR configuration tran & rot
...
void CTR::SolveBVP(const int &n_sample)
{
	...
	for (int i = 0; i < this->bc.m; ++i)
	{
		this->bvp.x_mesh.push_back(blaze::linspace<blaze::rowVector>(n_sample, this->bc.points[i], this->bc.points[i + 1]));
	}
